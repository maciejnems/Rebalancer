...
confs = [(conf.model_id, 2000) for conf in dex_exp.configs]
counter = -1
final_results = {}
...
for (id, len) in confs:
    ...
    final_results[id]["n"] = sys_model_result.iloc[counter].time.day
    final_results[id]["V"] = formulas.compute_V(sys_model_result.iloc[counter].pool)
